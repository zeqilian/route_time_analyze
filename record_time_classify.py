...
def classify_route_by_time(df, for_test=False):
    ...
    df.columns = [
        'time_range', 'route', 'max_duration', 'median_duration',
    ...
    time_route_df = df.sort_values(
        by=['time_range', 'median_duration'], ascending=[True, False])
    ...
    route_df = df.sort_values(
        by=['route', 'median_duration'], ascending=[True, True])
...
def classify_vehicle_by_time(df, for_test=True):
    ...
    df = df.groupby(['date', 'time_range', 'vehicle_id']).agg({
        'route': 'count'
    }).reset_index()
...
def classify_vehicle_by_time_detail(df, for_test=False):
    ...
    eng_vo_df = df[(df.trip_type == 'ENGINEER_TESTING') &
                   (df.vo_ticket.notnull())].copy()
    eng_no_vo_df = df[(df.trip_type == 'ENGINEER_TESTING') &
                      (df.vo_ticket.isnull())].copy()
    other_df = df[df.trip_type != 'ENGINEER_TESTING'].copy()

    eng_vo_df = classify_vehicle_by_time(eng_vo_df, for_test).rename(
        columns={
            'total_count': 'eng_vo'
        })
    eng_no_vo_df = classify_vehicle_by_time(eng_no_vo_df, for_test).rename(
        columns={
            'total_count': 'eng_no_vo'
        })
    other_df = classify_vehicle_by_time(other_df, for_test).rename(
        columns={
            'total_count': 'other'
        })
    all_df = all_df.merge(
        eng_vo_df, on=['time_range', 'vehicle_id'], how='left')
    all_df = all_df.merge(
        eng_no_vo_df, on=['time_range', 'vehicle_id'], how='left')
